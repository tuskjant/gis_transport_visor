...
export class ICGCGeocodingService implements GeocodingService {
    ...
    readonly errorPoint: GeocodedPoint[] = [
        {
            textAddress: "No s'ha localitza cap adreça",
    ...
    public async forwardGeocoding(
        ...
        var urlQuery: string = `text=${encodeURIComponent(
            textAddress
        )}&layers=${this.layers}&size=${this.size}`;
        if (focus !== null) {
    ...
    public async autocomplete(
        ...
        var urlQuery: string = `text=${encodeURIComponent(text)}&layers=${
            this.layers
